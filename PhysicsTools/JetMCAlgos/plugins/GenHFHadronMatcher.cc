                                                      ...
                                                      std::vector<int> &hadFromTopWeakDecay, std::vector<int> &hadBHadronId )
{
    ...
    // Find top quark and W boson
    // Finding b-quarks kinematically consistent with t->Wb decay
    for ( unsigned int hadNum=0; hadNum<nHad; hadNum++ ) {
        // Getting the last b-quark in the chain
        int qIdx = LastQuarkIds.at(hadNum).at(lastQuarkIndices.at(hadNum));
        reco::GenParticle& b_quark = hadMothers.at(qIdx);
        if (b_quark.pdgId() != 5) continue;
        std::cout << "pdgId: " << b_quark.pdgId() << "  p4: " << b_quark.p4() << std::endl;
    }
