...
EcalCoder_Ph2::EcalCoder_Ph2( bool                  addNoise     , 
		      bool                  PreMix1      ,
		      EcalCoder_Ph2::Noisifier* ebCorrNoise0 ,
		      EcalCoder_Ph2::Noisifier* ebCorrNoise1 ) :
   m_peds        (           nullptr ) ,
...
void  
EcalCoder_Ph2::setPedestals( const EcalLiteDTUPedestals* pedestals ) 
{
   m_peds = pedestals ;
}

void  
EcalCoder_Ph2::setGainRatios( const EcalCATIAGainRatios* gainRatios ) 
{
   m_gainRatios = gainRatios ; 
}

void 
EcalCoder_Ph2::setIntercalibConstants( const EcalIntercalibConstantsMC* ical ) 
{
   m_intercals = ical ;
}

double 
EcalCoder_Ph2::fullScaleEnergy( const DetId & detId ) const 
{
...
void 
EcalCoder_Ph2::analogToDigital( CLHEP::HepRandomEngine* engine,
...
void 
EcalCoder_Ph2::encode( const EcalSamples& ecalSamples , 
                   EcalDataFrame&     df,
...
void 
EcalCoder_Ph2::findPedestal( const DetId & detId  , 
			 int           gainId , 
			 double&       ped    , 
			 double&       width     ) const
{
   ...
   { 
      edm::LogError("EcalCoder_Ph2") << "Could not find pedestal for " << detId.rawId() << " among the " << m_peds->getMap().size();
   } 


   LogDebug("EcalCoder_Ph2") << "Pedestals for " << detId.rawId() << " gain range " << gainId << " : \n" << "Mean = " << ped << " rms = " << width;
}
...
void 
EcalCoder_Ph2::findIntercalibConstant( const DetId& detId, 
				   double&      icalconst ) const
{
   ...
   {
      edm::LogError("EcalCoder_Ph2") << "No intercalib const found for xtal " << detId.rawId() << "! something wrong with EcalIntercalibConstants in your DB? ";
   }
   icalconst = thisconst;
}
