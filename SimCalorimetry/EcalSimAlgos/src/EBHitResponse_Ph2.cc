...
void
EBHitResponse_Ph2::initialize(CLHEP::HepRandomEngine* engine)
{
...
EBHitResponse_Ph2::apdParameters() const
{
...
const CaloVShape*
EBHitResponse_Ph2::apdShape() const
{
...
void
EBHitResponse_Ph2::putAPDSignal( const DetId& detId  ,
			     double       npe    ,
			     double       time     )
{
...
//	    <<", nonlFunc="<<nonlFunc( npe*energyFac )<<std::endl ;
   ...
   if(!m_isInitialized) {
      throw cms::Exception("LogicError")
        << "EBHitResponse_Ph2::putAPDSignal called without initializing\n";
   }
...
double 
EBHitResponse_Ph2::apdSignalAmplitude( const PCaloHit& hit, CLHEP::HepRandomEngine* engine ) const
{
...
void 
EBHitResponse_Ph2::setIntercal( const EcalIntercalibConstantsMC* ical )
{
   m_intercal = ical ;
}

void 
EBHitResponse_Ph2::findIntercalibConstant( const DetId& detId, 
				       double&      icalconst ) const
{
   ...
   {
      edm::LogError( "EBHitResponse_Ph2" ) << 
	 "No intercal constant defined for EBHitResponse_Ph2" ;
   }
   else
   {
	 ...
	 if ( thisconst == 0. ) thisconst = 1. ; 
      ...
      {
	 edm::LogError("EBHitResponse_Ph2") << "No intercalib const found for xtal " 
					<< detId.rawId() 
...
void 
EBHitResponse_Ph2::initializeHits() {
...
void 
EBHitResponse_Ph2::finalizeHits() {
   const unsigned int bSize ( EBDetId::kSizeForDenseIndexing ) ;
...
void 
EBHitResponse_Ph2::add( const PCaloHit& hit, CLHEP::HepRandomEngine* engine )
{
...
void 
EBHitResponse_Ph2::run( MixCollection<PCaloHit>& hits, CLHEP::HepRandomEngine* engine )
{
   if( !index().empty() ) blankOutUsedSamples() ;
...
unsigned int
EBHitResponse_Ph2::samplesSize() const
{
   return m_vSam.size() ;
}

unsigned int
EBHitResponse_Ph2::samplesSizeAll() const
{
   return m_vSam.size() ;
}

const EcalHitResponse_Ph2::EcalSamples* 
EBHitResponse_Ph2::operator[]( unsigned int i ) const
{
   return &m_vSam[ i ] ;
}

EcalHitResponse_Ph2::EcalSamples* 
EBHitResponse_Ph2::operator[]( unsigned int i )
{
   return &m_vSam[ i ] ;
}

EcalHitResponse_Ph2::EcalSamples* 
EBHitResponse_Ph2::vSam( unsigned int i )
{
   return &m_vSam[ i ] ;
}

EcalHitResponse_Ph2::EcalSamples* 
EBHitResponse_Ph2::vSamAll( unsigned int i )
{
   return &m_vSam[ i ] ;
}

const EcalHitResponse_Ph2::EcalSamples* 
EBHitResponse_Ph2::vSamAll( unsigned int i ) const
{
   return &m_vSam[ i ] ;
}
