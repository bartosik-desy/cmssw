...
EcalDigiProducer_Ph2::EcalDigiProducer_Ph2( const edm::ParameterSet& params, edm::ProducesCollector producesCollector, edm::ConsumesCollector& iC) :
  EcalDigiProducer_Ph2(params, iC)
{
   if(m_apdSeparateDigi) producesCollector.produces<EBDigiCollectionPh2>(m_apdDigiTag);
...
EcalDigiProducer_Ph2::EcalDigiProducer_Ph2( const edm::ParameterSet& params,  edm::ConsumesCollector& iC) :
   ...
   m_ParameterMap     ( new EcalSimParameterMap_Ph2(
   ...
   m_APDResponse ( !m_apdSeparateDigi ? nullptr :
		   new EBHitResponse_Ph2( m_ParameterMap.get()  ,
				      &m_EBShape      ,
				      true            ,
				      m_apdParameters.get() ,
				      &m_APDShape       ) ) ,
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   m_ElectronicsSim.reset( new EcalElectronicsSim_Ph2( m_ParameterMap.get()    ,
                                                   m_Coder.get()           ,
                                                   applyConstantTerm ,
                                                   rmsConstantTerm     ) );
   ...
   {
     m_APDCoder.reset( new EcalCoder_Ph2( false            , 
                                      m_PreMix1        ,
                                      m_EBCorrNoise[0].get() ,
                                      m_EBCorrNoise[1].get() ));

     
     m_APDElectronicsSim.reset( new EcalElectronicsSim_Ph2( m_ParameterMap.get()    ,
                                                        m_APDCoder.get()        ,
                                                        applyConstantTerm ,
                                                        rmsConstantTerm     ) );
     
     m_APDDigitizer.reset( new EBDigitizer_Ph2( m_APDResponse.get()       , 
...
void
EcalDigiProducer_Ph2::initializeEvent(edm::Event const& event, edm::EventSetup const& eventSetup) {
...
void
EcalDigiProducer_Ph2::accumulateCaloHits(HitsHandle const& ebHandle, int bunchCrossing) {
...
void
EcalDigiProducer_Ph2::accumulate(edm::Event const& e, edm::EventSetup const& eventSetup) {
  // Step A: Get Inputs
...
void
EcalDigiProducer_Ph2::accumulate(PileUpEventPrincipal const& e, edm::EventSetup const& eventSetup, edm::StreamID const& streamID) {
  // Step A: Get Inputs
  edm::Handle<std::vector<PCaloHit> > ebHandle;
  if(m_doEB) {
    edm::InputTag ebTag(m_hitsProducerTag, "EcalHitsEB");
...
void
EcalDigiProducer_Ph2::beginLuminosityBlock(edm::LuminosityBlock const& lumi, edm::EventSetup const& setup)
{
   edm::Service<edm::RandomNumberGenerator> rng;
   if ( ! rng.isAvailable() ) {
...
void  
EcalDigiProducer_Ph2::checkCalibrations(const edm::Event& event, const edm::EventSetup& eventSetup ) 
{
...
void 
EcalDigiProducer_Ph2::checkGeometry( const edm::EventSetup & eventSetup ) 
{
